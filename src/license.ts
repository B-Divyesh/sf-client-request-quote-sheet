...
export const billingProductSlug = 'client-request-quote-sheet-studio';
...
export const checkoutUrl = `${billingBaseUrl}/api/v1/products/${billingProductSlug}/checkout`;
...
export async function verifyLicense(state: LicenseState, force = false): Promise<LicenseState> {
  ...
  try {
    const response = await fetch(`${billingBaseUrl}/api/v1/products/${billingProductSlug}/verify?license=${encodeURIComponent(state.token)}`, { headers: { Accept: 'application/json' } });
